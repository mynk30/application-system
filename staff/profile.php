...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_profile'])) {
    ...
    if (!$hasError) {
        try {
            ...
            $logger->info("Staff profile update success: " . ($updateSuccess ? 'Yes' : 'No'));
            ...
            if ($file_uploaded) {
                ...
                if (move_uploaded_file($file['tmp_name'], $file_path)) {
                    try {
                        ...
                        $deleteStmt = $conn->prepare("SELECT file_path FROM files WHERE model_type = 'staff' AND model_id = ?");
                        $deleteStmt->bind_param("i", $user_id);
                        $deleteStmt->execute();
                        $oldFile = $deleteStmt->get_result()->fetch_assoc();
                        
                        $deleteStmt = $conn->prepare("DELETE FROM files WHERE model_type = 'staff' AND model_id = ?");
                        $deleteStmt->bind_param("i", $user_id);
                        $deleteStmt->execute();
            ...
            $logger->error("Staff profile update failed: " . $e->getMessage());
