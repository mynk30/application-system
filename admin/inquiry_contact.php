...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    ...
    $stmt->close();
    header('Location: inquiry_contact.php');
    exit();
}

// Fetch all inquiries - Fixed query with proper ORDER BY syntax
$query = "SELECT * FROM contact_form ORDER BY created_at DESC";
$logger->info('Fetching all inquiries: ' . $query);
...
$logger->info('Fetched all inquiries successfully');
?>
                        ...
                        <tbody>
                            ...
                            <?php while ($row = $result->fetch_assoc()): ?>
                                <tr>
                                    ...
                                    <td><?= !empty($row['subject']) ? htmlspecialchars($row['subject']) : 'No Subject' ?></td>
                                    <td>
                                        <?php 
                                        $message = $row['message'] ?? '';
                                    ...
                                    <td>
                                        <button type="button" 
                                                ...
                                                onclick="if(confirm('Are you sure you want to delete this inquiry?')) { window.location.href='inquiry_contact.php?delete=<?= $row['id'] ?>' }"
                                                title="Delete">
